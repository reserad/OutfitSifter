...
public class cam_jewlery extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.jewelry_cam);
		...
		Toast.makeText(context, "Use a contrasting solid colored background in a well lighted area when taking pictures of jewelry", Toast.LENGTH_LONG).show();
		...
		preview.setOnTouchListener(new SurfaceView.OnTouchListener(){
			...
			public boolean onTouch(View arg0, MotionEvent arg1) {
				...
				final ImageView iv = new ImageView(cam_jewlery.this);
			...
			public void onClick(View arg0) {
				...
				capture.postDelayed(new Runnable() {
					...
					public void run() 
					{
						...
						{
							File destination = new File(photoPathName);
							Toast.makeText(context, (photoPathName), Toast.LENGTH_SHORT).show();
							Bitmap bmp = BitmapFactory.decodeFile(photoPathName);
							jewelery_crop j = new jewelery_crop();
							Bitmap bmp2 = j.selection(bmp);
							bmp.recycle();
							ByteArrayOutputStream bytes = new ByteArrayOutputStream();
							bmp2.compress(Bitmap.CompressFormat.PNG, 60, bytes);
							destination.delete();
						...
						{
						Toast.makeText(getApplicationContext(), e.toString() + "2" , Toast.LENGTH_SHORT).show();
						}
					}
				}, 4000);
	...
	PictureCallback myPictureCallback_JPG = new PictureCallback(){
		...
		class SavePhotoTask extends AsyncTask<byte[], String, String> {
            ...
            {
				File d = new File(Environment.getExternalStorageDirectory() + "/OutfitSifter/" + UserName + "/" + pathExtension + "jewlery/");
                String name = "";
                int count = 1;
                if(!d.exists())
                {
			...
			private void initPreview(int width, int height) {
				if (camera != null && previewHolder.getSurface() != null) {
					try {
						...
						Toast.makeText(cam_jewlery.this, t.getMessage(),
