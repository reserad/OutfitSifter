...
public class cam_shoes extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.shoes_cam);
		...
		preview.setOnTouchListener(new SurfaceView.OnTouchListener(){
			...
			public boolean onTouch(View arg0, MotionEvent arg1) {
				...
				final ImageView iv = new ImageView(cam_shoes.this);
	...
	PictureCallback myPictureCallback_JPG = new PictureCallback(){
		...
		class SavePhotoTask extends AsyncTask<byte[], String, String> {
			...
			protected String doInBackground(byte[]... png) {
				File d = new File(Environment.getExternalStorageDirectory() + "/OutfitSifter/" + UserName + "/" + pathExtension + "shoes/");
				String name = "";
				int count = 1;
				if(!d.exists())
                {
		...
		AutoFocusCallback myAutoFocusCallback = new AutoFocusCallback(){
			...
			private void initPreview(int width, int height) {
				if (camera != null && previewHolder.getSurface() != null) {
                    ...
                    {
						Toast.makeText(cam_shoes.this, t.getMessage(),
